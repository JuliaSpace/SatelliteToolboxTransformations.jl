...
!!! note
    The input file **must be exactly the same** as provided by IERS in CSV
    format. One can download it using the following commands:
...
A structure ([`EopIau1980`](@ref) or [`EopIau2000A`](@ref), depending
on `data_type`) with the interpolations of the EOP. Notice that the
