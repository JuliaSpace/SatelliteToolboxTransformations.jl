...
@testset "EOP field validation" begin
    create_itp = SatelliteToolboxTransformations._create_iers_eop_interpolation
    ...
    @test_throws ArgumentError create_itp(knots, [1.0, NaN, 3.0])

    # A missing knot inside the tabulated span is rejected, but a trailing one is tolerated
    # when the field is also missing there.
    @test_throws ArgumentError create_itp([2.4e6, NaN, 2.4e6 + 2], [1.0, 2.0, 3.0])
    itp = create_itp([2.4e6, 2.4e6 + 1, NaN], [1.0, 2.0, NaN])
